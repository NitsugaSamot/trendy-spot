const validation = (req, res, next) => {
  ...
  else if (name.length < 3) return res.status(400).json({error: "Write at least 3 letters"});
  else if (name.length > 50) return res.status(400).json({error: "Write up to 50 letters"});

  if (!size) return res.status(400).json({error: "Insert a size"});
  
  if (!price) return  res.status(400).json({error: "Insert a price"});
  else if (price < 100 || price > 100000) return  res.status(400).json({error: "the price must be between 100 and 100000"});
  else if (isNaN(price)) return  res.status(400).json({error: "Should be a number"});

  if (!image) return  res.status(400).json({error: "You must provide an image"});

  if (!description) return  res.status(400).json({error: "You must write a description"});
  else if ( description.length < 10 || description.length > 200) return  res.status(400).json({error: "The description must have between 10 and 200 caracters"});

  if (!stock) return  res.status(400).json({error: "You must insert to stock available"});
  else if (isNaN(stock)) return  res.status(400).json({error: "Should be a number"});
  else if (stock < 1 || stock > 1000) return  res.status(400).json({error: "The stock must be between 1 and 1000"});

  if (!color) return  res.status(400).json({error: "Choose a color!"});
  else if ( color.length < 3 || color.length > 20) return  res.status(400).json({error: "The color must be between 3 and 20 caracters"});

  if (!brand) return  res.status(400).json({error: "Write a brand"});
  else if ( brand.length < 3 || brand.length > 20) return  res.status(400).json({error: "The brand must have be between 3 and 20 caracters"});
